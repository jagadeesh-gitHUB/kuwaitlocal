...
angular.module('angucomplete-alt', [] ).directive('angucompleteAlt', ['$parse', '$http', '$sce', '$timeout' , '$rootScope', function ($parse, $http, $sce, $timeout, $rootScope) {
  var KEY_DW = 40,
      KEY_UP = 38,
      KEY_ES = 27,
      KEY_EN = 13,
  ...
  return {
    ...
    template:
      ...
      '    <label class="angucomplete-row" ng-repeat="result in results" ng-click="selectResult(result)" ng-mouseover="hoverRow($index)" ng-class="{\'angucomplete-selected-row\': $index == currentIndex}">' +
...
//console.log(typeof(scope.localData));
            matches = [];
...
if(scope.localData[i]!=null){
              for (s = 0; s < searchFields.length; s++) {

            //  console.log(scope.localData[i][searchFields[s]]);
              if(scope.localData[i][searchFields[s]]!=null){
             // console.log("jagade");
    ...
    /*  scope.selecttag=function(result){
      ...
      console.log("**************jagadeesh*******************");
        ...
        //Broadcast the result to native controllers
        ...
        scope.showDropdown = false;
        scope.results = [];
        //$rootScope.$broadcast('listingClick', returnData);
        $rootScope.dataHolder = returnData;
      };
